...
import { useBurnwTonTx } from "./burnwTonTx";

interface BurnwTonFlowProps extends HTMLAttributes<HTMLDivElement> {
  step: number;
  setStep: (value: SetStateAction<number>) => void;
  baseCoin: string;
}

const BurnwTonFlow: FC<BurnwTonFlowProps> = ({ setStep, step, baseCoin }) => {
  const [testHash, setTestHash] = useState<TxReq | undefined>();
  const [ethTxHash, setEthTxHash] = useState<string>();
  const { onFormSubmit } = useBurnwTonTx(setEthTxHash);
